...
void setup() {
  ...
  port2 = new Serial(this,"/dev/ttyUSB0",9600);
  port2.clear();
...
void draw() { 
  ...
  if(zumo_id ==2){
    ...
    // グラフなどの描画
    fill(red,0,0); rect(width/2+10, height/2, 100, -red);
    fill(0,green,0); rect(width/2+120, height/2, 100,-green);
    fill(0,0,blue); rect(width/2+230,height/2, 100,-blue);
   
    fill(red, green, blue); rect(width/2+350,30,200,200);

    if(myString1 != null){
      text(myString1, width/2+10, 10);                                      // シリアル通信で受信したテキストの表示
    }
  }
